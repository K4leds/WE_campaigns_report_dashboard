...
def render_health_dashboard(health_df: pd.DataFrame, entity_col: str, entity_label: str, key_prefix: str) -> str | None:
    ...
    with col1:
        ...
        for _, row in health_df.head(5).iterrows():
            with st.container():
    ...
    with col2:
        ...
        st.caption(f"Only currently *Running* {entity_label.lower()}s are shown here — there's nothing to act on for an Ended or Paused {entity_label.lower()}.")
        needs_attention_pool = health_df[health_df['Status'] == 'Running'] if 'Status' in health_df.columns else health_df
        bottom = needs_attention_pool[needs_attention_pool['Health Score'] < 60].head(5)
        if not bottom.empty:
    ...
    if selected_name:
        ...
        fig_radar.update_layout(
            polar=dict(
                ...
                angularaxis=dict(gridcolor='lightgray', gridwidth=1),
            ),
            showlegend=True,
