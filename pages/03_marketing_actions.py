...
@st.cache_data
def _cached_type_comparison(filtered_df_json):
    """Section 2: Journey vs One-Time type comparison."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    conv_col = 'Selected Conversions' if 'Selected Conversions' in filtered_df.columns else 'Unique Conversions'
...
@st.cache_data
def _cached_day_of_week(filtered_df_json):
    """Section 3: Day-of-week aggregation with per-day averages."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    conv_col = 'Selected Conversions' if 'Selected Conversions' in filtered_df.columns else 'Unique Conversions'
    ...
    df_dow['DayNum'] = pd.to_datetime(df_dow[date_col]).dt.dayofweek
...
@st.cache_data
def _cached_campaign_tags(filtered_df_json):
    """Section 4: Campaign tag parsing + explode + groupby."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    conv_col = 'Selected Conversions' if 'Selected Conversions' in filtered_df.columns else 'Unique Conversions'
...
@st.cache_data
def _cached_campaign_performance(filtered_df_json):
    """Section 5: Campaign-level aggregation with dynamic columns."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    conv_col = 'Selected Conversions' if 'Selected Conversions' in filtered_df.columns else 'Unique Conversions'
...
@st.cache_data
def _cached_segment_performance(filtered_df_json):
    """Section 6: Segment performance ranking."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    conv_col = 'Selected Conversions' if 'Selected Conversions' in filtered_df.columns else 'Unique Conversions'
...
@st.cache_data
def _cached_monthly_aggregation(filtered_df_json):
    """Section 7: Monthly aggregation with MoM growth rates."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    conv_col = 'Selected Conversions' if 'Selected Conversions' in filtered_df.columns else 'Unique Conversions'
...
@st.cache_data
def _cached_monthly_channel(filtered_df_json):
    """Section 7 (sub): Monthly channel revenue breakdown."""
    filtered_df = read_cached_json(filtered_df_json)
    rev_col = 'Selected Revenue (SAR)' if 'Selected Revenue (SAR)' in filtered_df.columns else 'Revenue (SAR)'
    date_col_m = 'Reporting Period Start Date' if 'Reporting Period Start Date' in filtered_df.columns else 'Day'
...
if 'Channel' in filtered_df.columns and not filtered_df.empty:
    chan_eff = _cached_channel_efficiency(filtered_df.to_json())
...
if 'Type of Campaign' in filtered_df.columns and not filtered_df.empty:
    type_comparison = _cached_type_comparison(filtered_df.to_json())
...
if 'Campaign Tags' in filtered_df.columns and not filtered_df.empty:
    tag_perf = _cached_campaign_tags(filtered_df.to_json())
...
if not filtered_df.empty:
    camp_perf = _cached_campaign_performance(filtered_df.to_json())
...
if 'Segment Name' in filtered_df.columns and not filtered_df.empty:
    seg_perf = _cached_segment_performance(filtered_df.to_json())
...
if date_col_m in filtered_df.columns and not filtered_df.empty:
    monthly_agg = _cached_monthly_aggregation(filtered_df.to_json())
